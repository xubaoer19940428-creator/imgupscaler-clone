...
export function SiteFooter({ locale }: { locale: Locale }) {
  ...
  return <footer className="site-footer"><div className="site-footer-inner"><div className="footer-brand">Img.Upscaler</div><p>{copy.copyright}</p><div className="footer-links"><Link href={`${prefix}/privacy-policy`}>{copy.privacy}</Link><Link href={`${prefix}/terms`}>{copy.terms}</Link><Link href={`${prefix}/cookies`}>{locale === 'zh' ? 'Cookie 政策' : 'Cookies'}</Link><a href="https://imgupscaler.com/blog/" target="_blank" rel="noreferrer">{copy.blog}</a><a href="mailto:support@imgupscaler.com">{copy.contact}</a><span ref={languageRef} className="language-switcher"><button type="button" className="language-trigger" aria-haspopup="menu" aria-expanded={languageOpen} aria-label={copy.switchLanguage} onClick={() => setLanguageOpen((open) => !open)}><span>{copy.currentLanguage}</span><span className="language-chevron" aria-hidden="true" /></button>{languageOpen && <span className="language-menu" role="menu"><Link role="menuitem" href={alternatePath} onClick={() => setLanguageOpen(false)}>{copy.alternateLanguage}</Link></span>}</span></div></div></footer>
}
