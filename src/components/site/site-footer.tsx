...
export function SiteFooter({ locale }: { locale: Locale }) {
  ...
  const alternatePath = `${locale === 'zh' ? pathWithoutLocale : `/zh${pathname === '/' ? '' : pathname}`}${query}`
  ...
  return <footer className="site-footer"><div className="site-footer-inner"><div className="footer-meta"><div className="footer-brand">Img.Upscaler</div><p>{copy.copyright}</p></div><div className="footer-links"><Link href={`${prefix}/privacy-policy`}>{copy.privacy}</Link><Link href={`${prefix}/terms`}>{copy.terms}</Link><Link href={`${prefix}/cookies`}>{copy.cookies}</Link><a href="https://imgupscaler.com/blog/" target="_blank" rel="noreferrer">{copy.blog}</a><a href="mailto:support@imgupscaler.com">{copy.contact}</a><div ref={languageRef} className="footer-language"><button type="button" className="footer-language-button" aria-haspopup="menu" aria-expanded={languageOpen} aria-label={copy.switchLanguage} onClick={() => setLanguageOpen((open) => !open)}><span>{copy.currentLanguage}</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m6 9 6 6 6-6" /></svg></button>{languageOpen && <div className="footer-language-menu" role="menu"><Link role="menuitem" href={alternatePath} onClick={() => setLanguageOpen(false)}>{copy.alternateLanguage}</Link></div>}</div></div></div></footer>
}
